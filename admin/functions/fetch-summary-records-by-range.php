...
if (!isset($_GET['start'], $_GET['end']) || empty($_GET['start']) || empty($_GET['end'])) {
    die(json_encode(["error" => "Missing required parameters (start and end dates)."]));
}
...
if (!DateTime::createFromFormat('Y-m-d', $startDate) || !DateTime::createFromFormat('Y-m-d', $endDate)) {
    die(json_encode(["error" => "Invalid date format. Expected format: YYYY-MM-DD"]));
}
...
while ($faculty = sqlsrv_fetch_array($facultyStmt, SQLSRV_FETCH_ASSOC)) {
    ...
    // 2. Get all unique scheduled dates for this faculty in the date range
    $datesQuery = "
        SELECT DISTINCT CAST(start_date AS DATE) as schedule_date
        FROM Schedules
        WHERE 
            rfid_no = ?
            AND CAST(start_date AS DATE) BETWEEN ? AND ?
            AND type IN ('Lecture', 'Laboratory', 'Consultation Time')
    ";

    $datesStmt = sqlsrv_query($conn, $datesQuery, [$rfid_no, $startDate, $endDate]);
    if ($datesStmt !== false) {
    ...
    $attdQuery = "
        ...
        WHERE 
            rfid_no = ?
            AND CAST(date_logged AS DATE) BETWEEN ? AND ?
    ";

    $attdStmt = sqlsrv_query($conn, $attdQuery, [$rfid_no, $startDate, $endDate]);
    if ($attdStmt !== false) {
    ...
    $hoursStmt = sqlsrv_query($conn, $hoursQuery, [$rfid_no, $startDate, $endDate]);
    if ($hoursStmt !== false) {
    ...
    $workedStmt = sqlsrv_query($conn, $workedQuery, [$rfid_no, $startDate, $endDate]);
    if ($workedStmt !== false) {
...
$response = [
    ...
    "status" => empty($facultySummaries) ? "No attendance data available for any faculty member in this date range" : "Success"
];
