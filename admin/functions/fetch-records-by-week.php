...
// Input validation
if (!isset($_GET['week'], $_GET['year'], $_GET['rfid_no']) || empty($_GET['week']) || empty($_GET['year']) || empty($_GET['rfid_no'])) {
...
// 1. Get SCHEDULED sessions for this week
$schedQuery = "
    SELECT 
    ...
    WHERE 
        rfid_no = ?
        AND DATEPART(WK, start_date) = ?
        AND YEAR(start_date) = ?
        AND type IN ('Lecture', 'Laboratory', 'Consultation Time')
";

$schedParams = [$rfid_no, $week, $year];
...
$attdQuery = "
    ...
    WHERE 
        rfid_no = ?
        AND DATEPART(WK, date_logged) = ?
        AND YEAR(date_logged) = ?
";

$attdStmt = sqlsrv_query($conn, $attdQuery, [$rfid_no, $week, $year]);
if ($attdStmt === false) {
