...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    try {
        ...
        if (strtolower($status) === 'declined') {
            $notif_time = date('Y-m-d H:i:s'); // current timestamp
            $updateSql = "UPDATE Appointments 
                          SET agenda = ?, status = ?, start_time = ?, end_time = ?, 
                              notif_time = ?, is_read = 0, is_read_student = 0 
                          WHERE appointment_code = ?";
            $updateParams = array($agenda, $status, $start_time, $end_time, $notif_time, $appointment_code);
        } else {
            $updateSql = "UPDATE Appointments 
                          SET agenda = ?, status = ?, start_time = ?, end_time = ? 
                          WHERE appointment_code = ?";
