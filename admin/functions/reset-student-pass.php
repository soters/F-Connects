...
function resetPassword($rfid_no) {
    ...
    try {
        // Step 1: Select email from Student table using RFID number
        $query_select = "SELECT email FROM Students WHERE rfid_no = ?";
        ...
        if ($stmt_select === false || !sqlsrv_has_rows($stmt_select)) {
            throw new Exception("No Student found with the provided RFID number.");
        }
        ...
        // Step 2: Update the password in StudentAccount table
        $query_update = "UPDATE StudentAccount SET password = ? WHERE email = ?";
    ...
    header("Location: ../pages/admin-update-student.php?rfid_no=" . urlencode($rfid_no) . "&message=" . urlencode($message) . "&type=" . urlencode($type));
    exit();
}
