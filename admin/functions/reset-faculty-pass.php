...
function resetPassword($rfid_no) {
    ...
    try {
        ...
        $query_select = "SELECT email FROM Faculty WHERE rfid_no = ?";
        ...
        if ($stmt_select === false || !sqlsrv_has_rows($stmt_select)) {
            throw new Exception("No faculty found with the provided RFID number.");
        }
        ...
        $query_update = "UPDATE FacultyAccount SET password = ? WHERE email = ?";
    ...
    header("Location: ../pages/admin-update-faculty.php?rfid_no=" . urlencode($rfid_no) . "&message=" . urlencode($message) . "&type=" . urlencode($type));
    exit();
}
