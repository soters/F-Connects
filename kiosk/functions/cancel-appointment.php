...
if (isset($_GET['appointment_code'])) {
    ...
    if ($stmt === false) {
        ...
        header("Location: ../student/kiosk-cancel-appt.php?appointment_code=" . urlencode($appointment_code) . "&rfid_no=" . urlencode($rfid_no));
        exit();
    } else {
        // If the query was successful
        $_SESSION['success_message'] = "Your appointment has been successfully cancelled.";
        header("Location: ../student/kiosk-student.php?rfid_no=" . urlencode($rfid_no));
        exit();
    }
} else {
